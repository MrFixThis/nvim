...
M.secure_require = function(modules)
  ...
  local reporter = function()
    ...
    report = string.format('[Secure require -> (%d)]: %s\n%s=> %s', report_counter, location, report, final_msg)
    vim.notify(report, 3)
  end
  ...
  local sanitizer = function(name)
    return name:gsub('%p', '_'):lower()
  end
  local nilCounter = 0
  local org_name
  ...
  if mods_loaded[org_name] then
    mods_loaded = mods_loaded[org_name]
  end
  if nilCounter > 0 then
    return reporter, mods_loaded
  end
